...
static void xor_round_key(unsigned char *state, unsigned char *keys, int round) {
	xor_(state, keys + round*KEY_LENGTH, KEY_LENGTH);
}
...
static void expand_key(unsigned char *key, unsigned char *keys) {
	int bytes = KEY_LENGTH;             // The count of how many bytes we've created so far
  ...
  memcpy(keys, key, KEY_LENGTH);      // The first 16 bytes of the expanded key are simply the encryption key
  ...
  while (bytes<176) {       // Until we have 176 bytes of expanded key, we do the following:
	...
	xor_(t, keys + bytes - KEY_LENGTH, 4);            // We exclusive-or t with the four-unsigned char block 16 bytes before the new expanded key.
    ...
    for (j=0;j<3;j++) {
	  ...
	  xor_(t, keys + bytes - KEY_LENGTH, 4);            // We exclusive-or t with the four-unsigned char block n bytes before
      memcpy(keys+bytes,t,4);            // This becomes the next 4 bytes in the expanded key
      bytes+=4;                          // Keep track of how many expanded key bytes we've added
    }
  }
